...
public class AssimilatedUniverseAction extends AbstractGameAction {
    ...
    public void update() {
        if (this.duration == Settings.ACTION_DUR_XFAST) {
            ...
            group.group.addAll(Originium.originiumPile.group);
            if (group.isEmpty()) {
                this.isDone = true;
                return;
            }
            ...
            if (group.size() == amount) {
                for (int i = 0; i < amount; i++) {
                    ...
                    AbstractDungeon.actionManager.addCardQueueItem(new CardQueueItem(card, AbstractDungeon.getRandomMonster(), card.energyOnUse, true, true), true);
                }
                isDone = true;
                return;
            }
